...
def validate_data_asset(asset: Dict[str, Any]) -> bool:
    ...
    """
    required_fields = Asset2RequiredFields.get(asset.get("type", ""), [])
    ...
    for field in required_fields:
        ...
        for part in parts:
            if part not in d:
                console.print(
                    f"[bold red]Error: Missing required field '{field}' in data asset.[/bold red]"
                )
                return False
            d = d[part]
...
def validate_upload_domain(upload_domain: str) -> str:
    """
    ...
    if not upload_domain or upload_domain.endswith(".metaphor.io"):
        raise ValueError(
            "Upload domain must non-empty and not end with '.metaphor.io' it's the part before that"
        )

    if upload_domain.startswith("https://"):
        raise ValueError("Upload domain must not start with 'https://'")
